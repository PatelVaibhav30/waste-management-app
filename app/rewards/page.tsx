    ...
    const fetchUserDataAndRewards = async () => {
      ...
      try {
        ...
        if (userEmail) {
          ...
          if (fetchedUser) {
            setUser(fetchedUser)
            const fetchedTransactions = await getRewardTransactions(fetchedUser.id) as any
  ...
  const refreshUserData = async () => {
    if (user) {
      ...
      if (fetchedUser) {
        ...
        setBalance(Math.max(calculatedBalance as number, 0)) // Ensure balance is never negative
      }
    }
  }
